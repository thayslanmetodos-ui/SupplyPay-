...
export default function OperatorDashboard() {
  ...
  useEffect(() => {
    ...
    socket.on('notification', (data) => {
      ...
      if (activeTab === 'my-cpfs') fetchActiveCpfs();
    });
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full p-6 space-y-8">
        ...
        {activeTab === 'reports' && (
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-bold">Relatórios de Atividade</h3>
              <div className="flex items-center gap-2 bg-slate-800/50 p-1 rounded-xl">
                <button className="px-3 py-1 text-[10px] font-bold uppercase bg-brand-blue text-white rounded-lg">Hoje</button>
                <button className="px-3 py-1 text-[10px] font-bold uppercase text-slate-500 hover:text-white transition-colors">Semana</button>
              </div>
            </div>

            <div className="grid grid-cols-1 gap-4">
              <div className="bg-brand-card border border-slate-800/50 p-6 rounded-3xl space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Total Gasto</p>
                    <p className="text-3xl font-bold">R$ {(transactions.filter(t => t.status === 'COMPLETED' && !t.withdrawal_amount).reduce((acc, curr) => acc + curr.amount, 0)).toFixed(2)}</p>
                  </div>
                ...
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">+12% em relação a ontem</p>
              </div>

              <div className="bg-brand-card border border-slate-800/50 p-6 rounded-3xl space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Total Recuperado (Saques)</p>
                    <p className="text-3xl font-bold">R$ {(transactions.filter(t => t.status === 'COMPLETED' && t.withdrawal_amount).reduce((acc, curr) => acc + curr.withdrawal_amount, 0)).toFixed(2)}</p>
                  </div>
                ...
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">+5% em relação a ontem</p>
              </div>
            </div>

            <div className="bg-brand-card border border-slate-800/50 p-6 rounded-3xl space-y-6">
              <h4 className="font-bold text-sm uppercase tracking-widest text-slate-500">Distribuição de Bancos</h4>
              <div className="space-y-4">
                {[
                  ...
                  { name: 'Outros', count: activeCpfs.filter(c => !['Nubank', 'Inter'].includes(c.bank)).length, color: 'bg-slate-500' }
                ].map((bank, i) => (
                  <div key={i} className="space-y-2">
                    <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest">
                      <span>{bank.name}</span>
                      <span>{bank.count} CPFs</span>
                    </div>
                    <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                      <div 
                        className={`h-full ${bank.color}`} 
