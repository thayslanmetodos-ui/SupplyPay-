...
async function startServer() {
  ...
  app.post("/api/operator/complete-withdrawal", authenticate, (req: any, res) => {
    ...
    const transaction = db.prepare("SELECT supplier_id FROM transactions WHERE id = ?").get(transactionId) as any;
    ...
    if (status === 'FAILED') {
      db.prepare("UPDATE users SET status = 'ON' WHERE id = ?").run(transaction.supplier_id);
      
      io.to(`user_${transaction.supplier_id}`).emit("notification", {
        type: "WITHDRAWAL_FAILED",
        message: "Seu saque foi reprovado. Seu CPF está disponível novamente.",
        transactionId
      });
    } else {
      io.to(`user_${transaction.supplier_id}`).emit("notification", {
        type: "WITHDRAWAL_COMPLETED",
        message: `Saque de R$ ${withdrawalAmount} aprovado!`,
        transactionId
      });
    }
